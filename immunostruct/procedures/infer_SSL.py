...
__all__ = ["inference_SSL", "inference_comparative_SSL"]


def inference_SSL(config, model, data_loader, device, clinical_loader=None, return_raw_preds=False, fig_save_folder=None, optimal_threshold=None):
    model.eval()
    true_targets = []
    predicted_probs = []  # Store raw probabilities for ROC AUC calculation

    with torch.no_grad():
        for graph_data, sequence_data, target, peptide_property, _ in data_loader:
...
def inference_comparative_SSL(config, model, data_loader, device, clinical_loader=None, return_raw_preds=False, fig_save_folder=None, optimal_threshold=None):
    model.eval()
    true_targets = []
    predicted_probs = []  # Store raw probabilities for ROC AUC calculation

    with torch.no_grad():
        for graph_data_pair, sequence_data_pair, target, peptide_property_pair, _ in data_loader:
            ...
            _, _, _, _, final_output, _ = model.forward_comparative((graph_data_cancer, graph_data_wt),
